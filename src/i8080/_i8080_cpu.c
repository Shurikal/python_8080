...
void instr_0x01(i8080oObject *self) {
    ...
    printf("LXI B");
    #endif
    self->C = self->rom_data[self->PC + 1];
    self->B = self->rom_data[self->PC + 2];
    self->PC+=2;
}
...
void instr_0x02(i8080oObject *self) {
    ...
    printf("STAX B");
    #endif
    self->rom_data[self->B * 256 + self->C] = self->A;
}
...
void instr_0x06(i8080oObject *self) {
    ...
    printf("MVI B");
    #endif
    self->B = self->rom_data[self->PC + 1];
    self->PC+=1;
}
...
void instr_0x0a(i8080oObject *self) {
    ...
    printf("LDAX B");
    #endif
    self->A = self->rom_data[self->B * 256 + self->C];
}
...
void instr_0x11(i8080oObject *self) {
    ...
    printf("LXI D");
    #endif
    self->E = self->rom_data[self->PC + 1];
    self->D = self->rom_data[self->PC + 2];
    self->PC+=2;
}
...
void instr_0x12(i8080oObject *self) {
    ...
    printf("STAX D");
    #endif
    self->rom_data[self->D * 256 + self->E] = self->A;
}
...
void F1(i8080oObject *self)
{
    printf("From F1\n");
    // print current pc
    //PyObject *pc = PyObject_GetAttrString(self, "pc");
    printf("pc = %d", self->PC);
}

void F2(i8080oObject *self)
{
    printf("From F2\n");
}

void F3(i8080oObject *self)
{
    printf("From F3\n");
}

void F4(i8080oObject *self)
{
    printf("From F4\n");
}


void (*CPU_FUNCTIONS[4]) (i8080oObject *self) = {F1, F2, F3, F4};